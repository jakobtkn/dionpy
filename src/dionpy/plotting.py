...
def plot_monostatic_vs_r(permittivities, freq, r_fractions=None,
    ...
    if r_over_lam is None:
        r_over_lam = np.linspace(0.01, 2.0, 400)
    ...
    fig, ax = plt.subplots(figsize=(_TW / 2, _TW / 2 / _GOLDEN))
    ax.plot(r_over_lam, rcs_dB, color="k")
...
def plot_forward_bw(configs, bw_frac: float = 0.3, n_freqs: int = 400,
    ...
    n_cfg = len(configs)
    fig, axes = plt.subplots(1, n_cfg, figsize=(_TW, _TW / _GOLDEN))
    if n_cfg == 1:
        axes = [axes]
    ...
    for ax, cfg, color in zip(axes, configs, colors):
        ...
        x_axis = (freqs - f0) / BW
        rcs_xz = np.zeros(n_freqs)
        rcs_yz = np.zeros(n_freqs)
        ...
        for i, freq in enumerate(freqs):
            ...
            d_sc = b_TE[:, -1]
            k = sphere.k(-1)
            rcs_xz[i] = bistatic_rcs(k, 0.0, 0.0,       b_sc, d_sc)
            rcs_yz[i] = bistatic_rcs(k, 0.0, np.pi / 2, b_sc, d_sc)

        norm = np.pi * r3**2
        rcs_xz_dB = 10 * np.log10(np.maximum(rcs_xz / norm, 1e-20))
        rcs_yz_dB = 10 * np.log10(np.maximum(rcs_yz / norm, 1e-20))

        peak_idx = np.argmax(rcs_xz_dB)
        peak_f_GHz = freqs[peak_idx] / 1e9

        ax.plot(x_axis, rcs_xz_dB, color=color,
                label=r"$\sigma_{3D}(\theta=0^\circ,\;\phi=0^\circ)$ -- XZ")
        ax.plot(x_axis, rcs_yz_dB, color=color, linestyle="--",
                label=r"$\sigma_{3D}(\theta=0^\circ,\;\phi=90^\circ)$ -- YZ")
        ax.plot(x_axis[peak_idx], rcs_xz_dB[peak_idx], marker="*", markersize=8,
    ...
    fig.suptitle(
        title or (
            rf"Forward Scattering ($\theta=0^\circ$) -- XZ vs YZ"
